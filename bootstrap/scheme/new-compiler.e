...
(e1:define (dataflow:graph-add-state-edge! graph from-id to-id)
  (e1:let ((successors (dataflow:graph-get-successors graph))
    ...
    (e1:let* ((from-successors (e1:if (unboxed-hash:has? successors from-id)
                                 (unboxed-hash:get successors from-id)
                                 list:nil))
              (to-predecessors (e1:if (unboxed-hash:has? predecessors to-id)
                                 (unboxed-hash:get predecessors to-id)
                                 list:nil))
